...
public class CartController {
    ...
    @PatchMapping
    public ResponseEntity<String> updateCartItemCount(@RequestParam(name = "email") String email,
        ...
        try{
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("해당 상품 Id 없음");
        } catch (IllegalArgumentException e){
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("해당 사용자의 장바구니에 없는 상품임");
        }
    }
    ...
    @PostMapping("/orders")
    public ResponseEntity<String> orderCartItem(@RequestParam String email,
